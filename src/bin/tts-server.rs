...
fn handle_generate(state: &ServerState, stream: &mut TcpStream, request: &Request) -> Result<()> {
    ...
    let text = form
        ...
        .trim();
    if text.is_empty() {
        return send_text(stream, 400, "Bad Request", "text must not be empty\n");
    }
    let max_frames = form
        .get("max_frames")
    ...
    let generated = state.engine.generate_codes(
        text,
        GenerateOptions {
            speaker,
            language,
    ...
    let wav = speech.to_wav_bytes(wav_gain)?;
    ...
    let headers = vec![
        ...
        (
            ...
            format_seconds(repetition_penalty as f64),
        ),
        ("X-TTS-Do-Sample", do_sample.to_string()),
        ("X-TTS-Top-K", top_k.to_string()),
        ("X-TTS-Top-P", format_seconds(top_p as f64)),
        ("X-TTS-Temperature", format_seconds(temperature as f64)),
        ("X-TTS-Subtalker-Do-Sample", subtalker_dosample.to_string()),
        ("X-TTS-Subtalker-Top-K", subtalker_top_k.to_string()),
        (
            "X-TTS-Subtalker-Top-P",
            format_seconds(subtalker_top_p as f64),
        ),
        (
            "X-TTS-Subtalker-Temperature",
            format_seconds(subtalker_temperature as f64),
        ),
        ("X-TTS-Seed", seed.to_string()),
    ];
