...
fn main() -> qwen3_hip_runtime::Result<()> {
    ...
    let gamma = deterministic_gamma(cols);
    ...
    if max_abs > 5e-6 {
        return Err(Error::InvalidInput(format!(
            "RMSNorm mismatch: max_abs={max_abs}"
        )));
    }

    println!(
        "RMSNorm smoke OK: rows={rows}, cols={cols}, max_abs={max_abs}, first8={:?}",
        &actual[..8]
    );
    Ok(())
}
...
fn deterministic_gamma(len: usize) -> Vec<f32> {
    (0..len)
        .map(|idx| 0.75 + ((idx % 23) as f32) / 37.0)
        .collect()
}
